...
# إحساس الارتطام (5.2): علامة ارتطام فوق المُصاب لا تلوين لجسمه.
# التلوين الأبيض القديم كان يمزج لون الوحدة 55% نحو الأبيض، فتصير على الأرض
# الذهبية أو في الثلج أفتح من الأرض نفسها فتختفي، وكان يُخرجها من مخزن الأشكال.
const HIT_FLASH := 0.1           # مدة علامة الارتطام
# بوحدات رسم الوحدة نفسها (طولها 26) لا بالبكسل، وتُضرب في حجمها فتتناسب معها
const HIT_MARK_R := 7.0          # نصف قطرها الأول
const HIT_MARK_GROW := 5.5       # وكم تتسع حتى تنطفئ
const HIT_MARK_Y := -15.0        # ارتفاعها عن القدمين: وسط الصدر
...
# تعويض ما يفقده القتال المرئي من وقت (ترنّح وتفادٍ ودورات حركة) حتى لا تختلف
# نتيجة معركة خارج الشاشة عن مثيلتها أمام العين — وهذا هو هدف الوصف 5.4.5 نصاً.
# الرقم مقيس لا مقدَّر: 0.85 كان يعطي المستوى الإحصائي +15% على ثماني مواجهات،
# و 0.74 يجعل النسبة 1.00. مهم جداً مع 500 وحدة لأن معظم القتال يجري خارج الشاشة.
const LOD_STAT_DMG := 0.74
...
# ---------- إضاءة المشهد: شيدر خفيف (10) ----------
# يحل محل مستطيل الطقس المسطّح فلا يزيد أمر رسم واحداً، وبلا قراءة أي صورة.
const LIGHT_SHADER := true      # يمكن إطفاؤه فيرجع المستطيل المسطّح كما كان
const VIGNETTE := 0.30          # تعتيم الأطراف عند الركن
const VIGNETTE_FROM := 0.55     # من أي نصف قطر يبدأ التعتيم
const FLOOR_SHADE := 0.10       # فرق الإضاءة بين أعلى الشاشة وأسفلها
const VIGNETTE_NIGHT := 0.42    # الليل يستحق حفّة أقوى قليلاً
const FLOOR_SHADE_NIGHT := 0.14
# الجودة التلقائية كانت تُطفئ شيدر الإضاءة عند هبوط الإطارات وتُرجعه — فكانت
# الهالة السوداء حول الشاشة تختفي وتعود أمام اللاعب. **شيء يظهر ويختفي أسوأ من
# بضعة بالمئة.** فحُذف التبديل، وصار التخفيف بالدقة لا بإطفاء أثر مرئي (14).
#
# ---------- دقة العرض حسب التقريب (14) ----------
# كلما ابتعدت الكاميرا ظهر عدد أكبر من المربعات والوحدات، وصار كل شيء أصغر على
# الشاشة. فتُخفَّض دقة العرض حينها: العين لا تكاد تفرّق، والمكسب حقيقي على
# الأجهزة الضعيفة حيث تعبئة البكسلات هي العنق.
# **بخطوات ثابتة لا متدرجة:** تغيير حجم هدف العرض نفسه مكلف، فلو تغيّر مع كل
# حركة إصبع لأكل المكسب. ومعه هامش تبديل (hysteresis) حتى لا يتذبذب عند الحدّ.
const RES_SCALE := true             # يمكن إطفاؤه فيبقى العرض بالدقة الكاملة
const RES_STEPS := [1.0, 0.85, 0.72]        # خطوات الدقة، من الأعلى إلى الأدنى
const RES_ZOOM_AT := [1.8, 1.2]     # تحت 1.8 ننزل خطوة، وتحت 1.2 خطوتين
const RES_HYST := 0.15              # هامش التبديل بوحدات التقريب
...
# مخزن أشكال الوحدات (unit_mesh.gd): شكل الوحدة لا يعتمد على موضعها، فيُحوَّل مرة
# واحدة إلى مثلثات ثم تُرسم الوحدة بأمر واحد بدل 43 (14)
const MESH_CACHE := true
const MESH_CACHE_MB := 24.0    # سقف الذاكرة؛ ما بعده يُرسم بالطريقة القديمة
# هامش حول وحدات القطر عند ضبط حدود لوحتها: الوحدة تُرسم فوق موضعها (طولها 26
# وحدة رسم) وبعض التأثيرات تتجاوزها، فالهامش يضمن ألا يُقصّ شيء منها
const BAND_MARGIN := 48.0
# توزيع تحديث الوحدات على الإطارات (14): ما هو خارج الشاشة لا يُرى، فلا داعي
# لتحديثه في كل إطار. يُحدَّث مرة كل STAT_EVERY إطاراً بزمن متراكم، فتبقى النتيجة
# نفسها ويسقط ثلاثة أرباع العمل. وما على الشاشة يبقى كل إطار حتى لا تتقطّع حركته.
const STAT_EVERY := 4
# والتباعد كذلك: ما هو خارج الشاشة يُباعَد مرة كل أربع دورات بقوة مضاعفة أربعاً،
# فالنتيجة نفسها على المدى والعمل رُبع ما كان (14 و 4.3)
const SEP_OFF_EVERY := 4
# هامش تأخر الشبكة: أقصى ما تقطعه وحدة قبل أن يصلها دورها في إعادة البناء
const GRID_SLACK := 0.8
const AURA_RESCAN := 0.25    # كل كم ثانية يُعاد جمع أصحاب الهالات
