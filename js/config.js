...
// --- القتال الواقعي (القسم 5.4): كل أرقام النظام هنا ---
export const combatRealism = {
  enabled: true,

  // 5.4.1 بذرة الشخصية: القيم تُولَّد من seed ثابت لكل وحدة
  seed: {
    boldnessMin: 0.7,      boldnessMax: 1.3,      // الجرأة
    dodgeSkillMin: 0.7,    dodgeSkillMax: 1.3,    // مهارة التفادي
    reactionMin: 0.10,     reactionMax: 0.35,     // زمن رد الفعل بالثواني
    speedVariation: 0.07,  // ±7% على السرعة
    sizeVariation: 0.04    // ±4% على الحجم
  },

  // 5.4.2 أنواع الحركات: استعداد، تعافٍ، معامل ضرر، زيادة مدى، وقوس الإصابة
  moves: {
    quick:  { name: 'سريعة',  windup: 0.18, recovery: 0.25, damage: 0.7, range: 0,   arc: 0.8 },
    heavy:  { name: 'قوية',   windup: 0.45, recovery: 0.55, damage: 1.6, range: 0,   arc: 1.35,
              stagger: 0.4, push: 1 },
    thrust: { name: 'طعنة',   windup: 0.30, recovery: 0.35, damage: 1.0, range: 0.6, arc: 0.45 },
    spin:   { name: 'دائرية', windup: 0.50, recovery: 0.60, damage: 0.8, range: 0,   arc: 1.6,
              splash: 1.4, pushAllies: 0.6 }
  },
  // الدائرية للمحطِّم والأبطال فقط
  spinUnits: { heroes: ['smasher'], champions: true },

  // اختيار الحركة بنظام نقاط لا بعشوائية محضة
  score: {
    base:            { quick: 1.0, heavy: 0.8, thrust: 0.7, spin: 0.5 },
    targetRecovering: 2.2,   // الهدف في زمن تعافٍ: فرصة للضربة القوية
    targetFarther:    1.8,   // الهدف أبعد قليلاً: طعنة
    farGapTiles:      0.45,  // "أبعد قليلاً" = خرج عن مدى الهجوم بهذا المقدار (دون مدى الطعنة)
    crowdEnemies:     3,     // ثلاثة أعداء حوله أو أكثر: دائرية
    crowdRadius:      1.6,
    crowdBonus:       2.4,
    lowHealthRatio:   0.30,  // دمه أقل من 30%: ضربة سريعة حذرة
    lowHealthBonus:   1.6,
    boldnessWeight:   1.2,   // الجرأة ترفع نقاط القوية والدائرية
    randomness:       0.15   // 15% عشوائية على النتيجة النهائية
  },

  // 5.4.3 ردود الأفعال
  reaction: {
    visionArcDegrees: 120,   // لا ترى الضربة إلا من هذا القوس أمامها
    dodgeChance: 0.18,       // احتمال التفادي الأساسي
    dodgeDistance: 0.8,      // قفزة بالمربعات
    dodgeSeconds: 0.25,      // مدة المناعة أثناء التفادي
    dodgeCooldown: 1.2,      // بين تفاديين
    blockChance: 0.45,       // احتمال صدّ أصحاب الدروع (يُضرب بمعامل الزاوية)
    blockSeconds: 0.3,
    blockKnockback: 0.5,     // ارتداد المهاجم بالمربعات
    blockStagger: 0.25,      // ترنّح المهاجم بعد الصدّ
    blockers: { heroes: ['armored'], champions: ['hammers'], police: true },
  ...
  // قاعدة الزاوية: جوهر النظام
  angle: {
    frontDegrees: 60,        // نصف قوس الأمام (120° كاملة)
    sideDegrees: 120,        // نصف قوس الجانب
    front: { defense: 1.0, damage: 1.00 },
    side:  { defense: 0.5, damage: 1.10 },
    back:  { defense: 0.0, damage: 1.25 }
  },

  // 5.4.4 المواقف الطريفة
  bump: {
    allyStagger: 0.3,        // الارتداد يدفع الحليف فيترنّح الاثنان
    allyRadius: 0.6,
    corpseRollPerDamage: 0.02,  // مسافة تدحرج الجثة لكل نقطة ضرر قاتل
    corpseRollMax: 1.6,
    corpseSeconds: 1.0,
    corpseShove: 0.35,       // زحزحة من تصطدم به الجثة
    maxRollingBodies: 20
  },

  // 5.4.5 مستويات التفصيل
  detail: {
    fullUnits: 60,           // أقرب هذا العدد للكاميرا داخل الشاشة
    fullUnitsCrowded: 30,    // عند تجاوز الحد أدناه
    crowdedMatch: 800,
    recalcSeconds: 0.5,
    offScreenDamage: 0.85    // تعويض عن التفادي المتوسط خارج الشاشة
  }
};
